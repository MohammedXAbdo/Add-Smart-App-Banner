...
function asab_generateRandomString($length = 10) {
    return substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, $length);
}
function asab_my_admin_scripts() {
wp_enqueue_script('media-upload');
wp_enqueue_script('thickbox');
}
function asab_my_admin_styles() {
wp_enqueue_style('thickbox');
}
add_action('admin_print_scripts', 'asab_my_admin_scripts');
add_action('admin_print_styles', 'asab_my_admin_styles');
?> 
...
<?php
 if (isset($_POST["update_settings"])) {
    // Do the saving
    $asab_random = esc_attr($_POST["asab_random"]);  
	check_admin_referer( 'asab_settings'.$asab_random  );
...
?>
        <form method="POST" action="">
        	<?php
        	$asab_random=asab_generateRandomString();
        	wp_nonce_field( 'asab_settings'.$asab_random );
        	?>
          <input type="hidden" name="asab_random" value="<?php echo $asab_random; ?>" />
...
<tr>
    ...
    <br>
   for any help contact me <a href="https://facebook.com/mohammed.abdom2" target="_blank">@Mohammed Abdo</a>
       <br>
   for Donte <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=Q64BVQ2UVV3SS&source=url" target="_blank">@Paypal</a>
    </div>
    <script>
